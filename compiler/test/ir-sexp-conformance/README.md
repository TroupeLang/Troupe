...
`native` is the exception: a hand-built unit rather than a compiled program, so the corpus does
not depend on which native-module manifests a checkout has installed. Its body references native
modules and its document carries the `natives` header, pinning the header's canonical form
(sorted, deduplicated). It is regenerated by the same `IR_SEXP_REGENERATE=1` run and carries the
same cross-implementation counterparts as the compiled references.
