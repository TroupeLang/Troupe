...
- `Markdown`      : Markdown source to HTML: headings, paragraphs, code blocks, block quotes,
                    lists, thematic breaks, emphasis, code spans, links and images. Output is built
                    as `Html` nodes rather than concatenated strings, so escaping and dangerous-URL
                    blocking come from `Html`, and literal HTML in the source is escaped rather
                    than passed through.
...
- `Sexp`          : Reader and printer for Lisp-style s-expressions: `ATOM` / `STR` / `LST`,
                    ...
                    rendering.
- `SimpleFileIO`  : `readFile` and `writeFile` reporting failure as an `Outcome`
                    (`ERR {reason, path}`) instead of the runtime's tagged record. Import it
                    selectively: `import { readFile, writeFile } SimpleFileIO` shadows the
                    builtins of those names, leaving call sites unchanged.
