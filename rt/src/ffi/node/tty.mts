...
import { stdio_level, checkChannelEffect, suspendReadline } from '../../builtins/stdio.mjs';

/**
 * Tty — the terminal native module (manifest ffi/Tty.exports), consumed
 * through lib/Tty.trp.
 *
 ...
 * interaction with readline, an observation being no effect and nothing for
 * the sink check to refuse. The three operations are effects on the channel
 * and carry the sink check, as `fwrite` does — a subscription resumes the
 * stream and starts draining input from the OS buffer, which a later reader
 * can observe.
 *
 * Core stdio owns the terminal's channel state — `stdio_level`,
 * `checkChannelEffect`, the readline hand-off — and this module is a client of
 * it (builtins/stdio.mts): the channel level is one level however many parties
 * observe or write the channel.
 *
 ...
 * from Node callbacks, outside any thread, and this is how they reach the
 * mailbox and the scheduler.
 */
...
/**
 * Reset the terminal state the runtime is responsible for. Registered as this
 * module's cleanup with the native-module registry (ffi/node/index.mts) and
 * run from `cleanupAsync` (runtimeMonitored.mts), which runs for every
 * program, on every termination route that reaches it — so this must be safe
 * when nothing was set up, and safe to run twice.
 *
...
function ttyObservationLevel(): Level {
    return lub(getRuntimeObject().$t.pc, stdio_level);
}

function mkTtyOk(value: LVal): LVal {
    const rec = Record.mkRecord([
        ['tag', new LVal('Ok', ttyObservationLevel())],
        ['value', value],
    ]);
    return getRuntimeObject().$t.mkValWithLev(rec, stdio_level);
}

function mkTtyErr(reason: string): LVal {
    const lev = ttyObservationLevel();
 ...
 */
function ttyDescriptor(arg, admissible: any[], expected: string) {
...
export const ttyExports = {
    ...
    ttySize: mkBase((arg) => {
        ...
        if (fd.isTTY !== true) {
            return getRuntimeObject().ret(mkTtyErr("not a terminal"));
        }
        const cols = fd.columns;
        const rows = fd.rows;
        if (typeof cols !== 'number' || typeof rows !== 'number') {
            return getRuntimeObject().ret(mkTtyErr("terminal size unavailable"));
        }
        const lev = ttyObservationLevel();
        ...
        return getRuntimeObject().ret(mkTtyOk(new LVal(size, lev)));
    }, "ttySize"),
    ...
    ttyLevel: mkBase((arg) => {
        ...
        return getRuntimeObject().ret(getRuntimeObject().$t.mkVal(stdio_level));
    }, "ttyLevel"),
    ...
    ttyRawMode: mkBase((arg) => {
        ...
        checkChannelEffect(getRuntimeObject().$t, "terminal raw-mode change",
        ...
        try {
            ...
            return getRuntimeObject().ret(
                mkTtyErr(e instanceof Error ? e.message : String(e)));
        }
        return getRuntimeObject().ret(
            mkTtyOk(new LVal(__unitbase, ttyObservationLevel())));
    }, "ttyRawMode"),
    ...
    ttySubscribe: mkBase((arg) => {
        ...
        getRuntimeObject().$t.raiseCurrentThreadPC(
            lub(arg.lev, arg.val[0].lev, arg.val[1].lev));

        checkChannelEffect(getRuntimeObject().$t, "terminal subscription",
                           arg.lev, arg.val[0].lev, arg.val[1].lev)
        ...
        subscriberRuntime = getRuntimeObject();
        attachTtyListeners();
    ...
    ttyUnsubscribe: mkBase((arg) => {
        ...
        ttyDescriptor(arg, [process.stdin], "an input descriptor");

        checkChannelEffect(getRuntimeObject().$t, "terminal unsubscription",
                           arg.lev)
